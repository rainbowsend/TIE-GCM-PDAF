...
# required to avoid recompile of the whole PDAF binding when gitversion.inc changes
$(OBJDIR)/nc_functionality.o: $(BIND_SRC_DIR)/gitversion.inc
endif
...
PDAF_DIR_FOR_GITVERSION := $(patsubst -I%,%,$(INC_PDAF))

$(BIND_SRC_DIR)/gitversion.inc: .git/HEAD .git/index
	@{ \
	  echo "! This file is auto-generated by the TIEGCM build process"; \
	  echo "      character(len=40) :: gitversion_tiegcm_pdaf = '$(shell git rev-parse HEAD)'"; \
	  echo "      character(len=40) :: gitversion_pdaf_binding_tiegcm = '$(shell git -C $(BIND_SRC_DIR)/.. rev-parse HEAD 2>/dev/null || echo unknown)'"; \
	  echo "      character(len=40) :: gitversion_pdaf = '$(shell git -C $(PDAF_DIR_FOR_GITVERSION) rev-parse HEAD 2>/dev/null || echo unknown)'"; \
	  echo "      character(len=40) :: gitversion_geodetic_fortran_utilities = '$(shell git -C deps/geodetic-fortran-utilities rev-parse HEAD 2>/dev/null || echo unknown)'"; \
	  echo "      character(len=40) :: gitversion_esmf = '$(shell git -C $(ESMF_DIR) rev-parse HEAD 2>/dev/null || echo unknown)'"; \
	} > $@
