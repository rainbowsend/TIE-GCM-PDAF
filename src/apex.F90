...
!
! 12 SEP 2022: replaced Nonconforming tab characters
!              removed trailing white space characters
...
!   real, dimension(3,1) :: b_enu
!   real, dimension(3) :: b_sph
!   real, dimension(3) :: cart,sph
! #endif

  if (iflag == 1) then
    ! convert to cartesian coordinates (x,y,z) scaled in earth radii
    is   = 1
    rlat = glat*dtr
    ct   = sin(rlat)
    st   = cos(rlat)
    ...
    ! coordinates are already cartesian
    is  = 2
    xxx = glat
    yyy = glon
    zzz = alt
...
!
  if (iflag /= 3) then
    do i=1,last
      g(i) = gb(i) ! gb is module data from cofrm (magnetic field)
    enddo
  else
    do i=1,last
      g(i) = gv(i) ! gv is module data from cofrm (magnetic potential)
    enddo
  endif
...
!#ifdef USE_PDAF
!  ! TODO std only true for ground not at hights
!
!
!  ! Here B is always in cartesian coords
!   sph=cartesian2spherical((/xxx,yyy,zzz/))
!!    write(*,*) sph(1), '°E', sph(2),'°N', (sph(3)*re)-re,'km', sqrt(bxxx**2+byyy**2+bzzz**2)
!   R=Rxyz2enu(sph(1),sph(2))
!
!   b(1,1) = bxxx
!   b(2,1) = byyy
!   b(3,1) = bzzz
!
!!    write(*,*) is, ' B original: ', b(:,1)
!
!   call default_dgemm(R,b,b_enu)
!
!! perturb east north up
!! ATTENTION perturbation of components can cause instabilities
!! when using the same perturbations for all locations. Compared to the original value the perturbation can be much larger
!!    call param_igrfbe%handle(b_enu(1,1))
!!    call param_igrfbn%handle(b_enu(2,1))
!!    call param_igrfbu%handle(b_enu(3,1))
!
!   b_sph=cartesian2spherical(b_enu(:,1))
!!   write(*,*) 'B sph: ', b_sph
!   call param_igrfbd%handle(b_sph(1))
!   call param_igrfbi%handle(b_sph(2))
!   call param_igrfbf%handle(b_sph(3))
!!    write(*,*) 'B sph pert: ', b_sph
!   b_enu(:,1)=spherical2cartesian(b_sph)
!   call default_dgemm(R,b_enu,b,tA=.true.)
!
!   bxxx = b(1,1)
!   byyy = b(2,1)
!   bzzz = b(3,1)
!
!! ATTENTION already perturbed. Do not perturb twice
!!   if(param_igrfbf%handling/=PARAM_HANDLING_NONE)then
!!     babs = sqrt(bxxx**2+byyy**2+bzzz**2)
!!     bxxx=bxxx/babs
!!     byyy=byyy/babs
!!     bzzz=bzzz/babs
!!     call param_igrfbf%handle(babs)
!!     bxxx=bxxx*babs
!!     byyy=byyy*babs
!!     bzzz=bzzz*babs
!!   end if
!
!!  write(*,*) 'B perturbed: ', b(:,1)
!#endif

  babs = sqrt(bxxx**2+byyy**2+bzzz**2)

!   write(*,*) sph(1), '°E', sph(2),'°N', sph(3),'Earth radii', babs

  if (is .eq. 1) then            ! (convert back to geodetic)
    beast = byyy*cp-bxxx*sp ! B east
    brho  = byyy*sp+bxxx*cp
    bnrth = bzzz*st-brho*ct  ! B north
    bdown = -bzzz*ct-brho*st ! B down
...
!
    ...
    g2(81:n2,8) = 0.0
#ifdef USE_PDAF
  ! perturb SH coefficient
  ! IGRF uses Schmidt-normalized spherical harmonic coefficients for the potential function
  !
  ! B = - grad V
  !
  ! The Gauss coefficients gnm, hnm are provided in units of nano tesla
  ! the order of coefficents in the arrays is
  ! g10 g11 h11 | g20 g21 h21 g22 h22 | g30 g31 h31 g32 h32 g33 h33 | ...
  do n=1,ncn1
    g = 0.0
    g(1:n1) = g1(:,n)
    call param_igrf_sh%handle(g)
    g1(:,n) = g(1:n1)
  end do
  do n=1,ncn2
    call param_igrf_sh%handle(g2(:,n))
  end do
#endif
!
! Set gh from g1,g2:
